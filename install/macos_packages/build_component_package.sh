...
# Requirements:
#  - MacOS
#  - A lukso repository with binaries and documentation already built.
# Usage: run this script, passing $1 to the repository path. The script assumes
# it is being run from within a lukso repository if $1 is not provided.

usage() {
    echo "Builds a component package for macOS."
    echo "Assumes that the following items already exist:"
    echo "    - A lukso binary which has already been notarized"
    echo "    - Documentation created by \`npm run build\`, usually in a dist"
    echo "      directory at <repo>/public"
    echo "Usage: $0 <path-to-lukso-binary> <path-to-dist-directory>"
}
...
lukso_documentation_dir="$2"

if [ ! -f "$lukso_program_file" ]; then
    error "Could not find lukso binary at $lukso_program_file"
fi

if [ ! -d "$lukso_documentation_dir" ]; then
    error "Could not find lukso documentation at $lukso_documentation_dir"
fi

pkgdir="$(mktemp -d)"
mkdir -p "$pkgdir/usr/local/bin"
cp "$lukso_program_file" "$pkgdir/usr/local/bin/lukso"
...
"./$server_prog_name" --ip 127.0.0.1 --index "$lukso_documentation_dir" &
server_pid="$!"
...
mv 127.0.0.1:8000 "$pkgdir/usr/local/share/doc/lukso"
...
version="$(lukso_version "$lukso_program_file")"
pkgbuild --identifier com.luksoprompt.lukso --version "$version" --root $pkgdir lukso-component.pkg
